...
headers = {
    'User-Agent': 'Mozilla/5.0 (Linux; Android 13; V2053) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 Mobile Safari/537.36'
}
...
email_addr = data['email_addr'] # This is the variable you want to use
print(f"{email_addr}")


# Step 2: Poll inbox until message arrives
def check_inbox():
    params = {
        'f': 'check_email',
        'sid_token': sid_token,
...
# Step 3: Fetch email contents
def fetch_email_content(mail_id):
    params = {
        'f': 'fetch_email',
        'sid_token': sid_token,
        'email_id': mail_id
...
# Step 4: Extract href links from the email body
            ...
            try:
                ...
                if registration_error:
                    ...
                    for _ in range(3):  # Retry up to 3 times with same email
                        ...
                        data["email_addr"] = email_addr # CHANGED THIS LINE
    ...
    while True:
        ...
        if messages:
            for msg in messages:
                ...
                email_detail = fetch_email_content(mail_id)
                body = email_detail.get('mail_body', '')
